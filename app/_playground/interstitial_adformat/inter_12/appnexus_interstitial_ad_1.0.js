...
AppNexusAdTemplate.AppNexInterstitial = {
	...
	getImgSize: function() {
		...
		if (aat.conf._orientation == "portrait") {
			...
			if (aat.conf._adImages.length == 4) {
				...
				width = aat.conf._width_landscape + "px";
				backimg = aat.conf._adImages[0];
			}
		}
		img.src = backimg;
		...
		img.onload = function() {
			...
			if (aat.conf._orientation == "landscape" && aat.conf._adImages.length == 1) {
	...
	init: function() {
		...
		// TEST
		aat.doc.querySelector("#testdiv").innerHTML = "userAgent : " + aat.conf._userAgent + ", orientation : " + aat.conf._orientation + ", width : " + aat.conf._browser_width + ", height : " + aat.conf._browser_height;
