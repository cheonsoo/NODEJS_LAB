...
const txt = '11월 08 06:09:46 오전 ERROR - NewGoogleDataServiceImpl.exportCreateSchedule(405) |                           com.google.api.client.googleapis.json.GoogleJsonResponseException: 401 Unauthorized';
...
// var reg = /ERROR - .*?\(/gi;
// var reg = new RegExp('([0-11][1-12])월\\s(\\d\\d)\\s(\\d\\d):(\\d\\d):(\\d\\d)\\s(오전|오후)', 'gi');
var reg = new RegExp('\\d{2}월\\s(\\d{2})\\s(\\d{2})\\:(\\d{2})\\:(\\d{2})\\s(오전|오후)', 'gi');
