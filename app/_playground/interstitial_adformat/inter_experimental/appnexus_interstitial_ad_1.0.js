...
AppNexusAdTemplate.AppNexInterstitial = {
	...
	scaleImageSizeToOuterDiv : function() {
		var aat = this;

		// FIRE FOX. Currently this code only uses for Fire Fox
		if (aat.conf._userAgent[0] == "firefox") {
			var image = aat.doc.querySelector("." + aat.conf._interstitial_img_inner_clsname);
			var imageParent = aat.doc.querySelector("." + aat.conf._interstitial_outer_clsname);

			var parent_ratio = imageParent.offsetWidth / imageParent.offsetHeight;
			var img_ratio = image.width / image.height;
			if (aat.conf._imageRatio == -1) {
				aat.conf._imageRatio = img_ratio;
			}

			console.log("parent width : " + imageParent.offsetWidth);
			console.log("parent height : " + imageParent.offsetHeight);
			console.log("image width : " + image.width);
			console.log("image height : " + image.height);
			console.log("parent_ratio : " + parent_ratio);
			console.log("img_ratio    : " + img_ratio);
			console.log("------------------------------------------------------------------");

			if (parent_ratio < aat.conf._imageRatio) {
	...
	/*
	 * INIT PROPERTIES & ELEMENTS
	 */
	init: function() {
		var aat = this;

		// Find out if it's in iframe or not. For DX tag
		if (!aat.conf._preview == "PREVIEW" && aat.win.self != aat.win.top) {
...
/*
 * SAFER WAY TO USE ONLOAD.
 */
if (document.readyState !== "complete") {
    addEvent(window, "load", function() {
    	eval("loadInterstitial_" + "PREVIEW" + "()");
    });
}
